  ...
  asHTML() {
    return `
      <div class="list-element">
        <div>Multiplier: <span style="color: ${this.multiplierColorStr()}">${Math.round(this.multiplier * 1000) / 1000}</span></div>
...
function getMult(build) {
  return (BASE_HEALTH + HEALTH_PER_VIT * build.vit + build.stats[1]) / BASE_HEALTH * (build.stats[0] + BASE_ATTACK) / BASE_ATTACK * (includeSecondary ? otherMult(build) : 1);
}
...
function calculateCombinations(combinations, index, arr, remaining) {
  if (index === 6) {
  ...
  for (let i = 0; i <= remaining; i++) {
    arr[index] = i;
    calculateCombinations(combinations, index + 1, arr, remaining - i);
...
function solve(vit = 0, useSunken = false) {
        ...
        for (let j = 0; j < accessories2.length; j++) {
          ...
          for (const accessory3 of accessories3) {
            ...
            for (const item of armorList) {
              for (const k in armorStats)
                armorStats[k] += item.stats[k];
            }
            
            for (const combination of EnchantCombinations) {
              const stats = [0, 0, 0, 0, 0, 0];
              for (const i in stats) {
                stats[i] = armorStats[i] + combination[i] * Armors[4][i].stats[i];
              }
              const build = new Build(armorList, vit, stats, combination);
              if (!buildSet.contains(build))
                n++;
              if (build.isValid())
                buildSet.add(build);
              if (buildSet.size > SIZE * 10) {
  ...
  console.log("Num builds: " + buildSet.size, n);
  return purge(buildSet.toList());
}

// Sort and limit number of elements
function purge(builds) {
...
async function run() {
  ...
  calculateCombinations(EnchantCombinations, 0, [], 5);
