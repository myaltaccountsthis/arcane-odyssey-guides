...
export default function BuildComponent({ build }: Props) {
    return (
        <div className="border-2 border-solid border-black rounded-lg px-4 py-2 w-fit">
            <div title="Multiplier of all stats, scales with weight">Multiplier: <span style={{ color: getMultiplierColorStr(build.multiplier) }}>{getFormattedMultiplierStr(build.multiplier)}</span></div>
