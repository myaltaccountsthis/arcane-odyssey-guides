...
export default function BuildComponent({ build, decimals }: Props) {
    return (
        <div className="border-2 border-solid border-black rounded-lg px-4 py-2 w-fit">
            <div title="Multiplier of all stats, scales with weight">Multiplier: <span style={{ color: getMultiplierColorStr(build.multiplier) }}>{getFormattedNumberStr(build.multiplier, decimals)}</span></div>
            <div title="Multiplier from power and defense">Base Multiplier: <span style={{color: getMultiplierColorStr(build.baseMultiplier)}}>{getFormattedNumberStr(build.baseMultiplier, decimals)}</span></div>
            <div title="Total stats, normalized to power">Efficiency Points: <span style={{color: getEfficiencyPointsColorStr(getNormalizedStats(build.stats) / 80)}}>{getFormattedNumberStr(getNormalizedStats(build.stats), decimals)}</span></div>
